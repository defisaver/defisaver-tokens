 ...
 */
const handleWBTCLegacy = (symbol: string = ""): string =>
  symbol === "WBTC Legacy" ? "WBTC" : symbol;

const _addChainSpecificData = (
...
export const getAssetInfo = (
  ...
  let assetData = assets.find((t) =>
    compare(t.symbol, handleWBTCLegacy(symbol))
  );
  if (!assetData) assetData = { ...assetProto, symbol };
...
export const getIlkInfo = (ilk: string = ""): ExtendedIlkData => {
  const _ilk = (
    ilk.substr(0, 2) === "0x" ? bytesToString(ilk) : ilk
  ).toUpperCase();
  const ilkData =
    ilks.find((i) => i.ilkLabel === _ilk) ||
    reflexerCollTypes.find((i) => i.ilkLabel === _ilk);
  if (!ilkData) {
...
export const getAssetInfoByAddress = (
  address: string = "",
  chainId?: number
): AssetData => {
  const assetDataBase = assets.find(
    (t) =>
      t.addresses[chainId || config.network]?.toLowerCase() ===
      address.toLowerCase()
  );
...
export const ilkToAsset = (ilk: string = ""): string => {
  ...
  if (ilkLabel.startsWith("PSM"))
    asset = ilkLabel.match(/^[A-Z]*-(.*)-[A-Z]*/)?.[1] || asset;
...
/** @private **/
export const compoundAsset = (underlyingAsset: string): string =>
  `c${underlyingAsset.toUpperCase()}`;
/** @private **/
export const aaveAsset = (underlyingAsset: string): string =>
  `a${underlyingAsset.toUpperCase()}`;
 ...
 * @returns {string} Token symbol
 */
export const tokenFromMakerJoin = (join: string): string =>
  tokenFromJoin(join, ilks);

/**
 * @param join {string} Reflexer ilk join
 * @returns {string} Token symbol
 */
export const tokenFromReflexerJoin = (join: string): string =>
  tokenFromJoin(join, reflexerCollTypes);

export const getAaveV2MarketInfo = (name: string = ""): AaveMarketData | void =>
  aaveV2Markets.find((i) => i.name === name);
...
export const assetAmountInEth = (
  ...
  return new Dec((amount && amount.toString()) || 0)
    .div(10 ** decimals)
    .toDP(decimals)
    .toString();
};
